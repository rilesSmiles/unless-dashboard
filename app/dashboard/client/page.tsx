...
type StepTask = { id: string; is_done: boolean }
type Step = { id: string; title: string; step_order: number; project_step_tasks: StepTask[] }
type Project = { id: string; name: string; project_type: string | null; created_at: string; project_steps: Step[] }
...
function formatMoney(cents: number | null, currency: string | null) {
  ...
  try {
    return new Intl.NumberFormat('en-CA', {
      style: 'currency', currency: (currency || 'CAD').toUpperCase(), maximumFractionDigits: 0,
    }).format(cents / 100)
  } catch { return `$${((cents ?? 0) / 100).toFixed(0)}` }
}
...
export default function ClientDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Profile — snapshot last_seen_at BEFORE we update it
      ...
      // Use last login as cutoff; fall back to 30 days ago so new clients always see content
      const lastSeen = profileData?.last_seen_at
      ...
      const { data: projData } = await supabase
        ...
        .select(`id, name, project_type, created_at,
          project_steps(id, title, step_order,
      ...
      if (projectIds.length > 0) {
        ...
        // New published meeting notes since last login
        ...
        const combined: NewItem[] = [
          ...((docData ?? []) as any[]).map((d) => ({ ...d, type: 'document' as const })),
          ...((noteData ?? []) as any[]).map((n) => ({ ...n, type: 'note' as const })),
      ...
      // Update last_seen_at AFTER fetching new items
  ...
  const projectProgress = useMemo(() => {
    ...
    let total = 0, done = 0
    for (const s of steps) for (const t of s.project_step_tasks ?? []) { total++; if (t.is_done) done++ }
  ...
  const outstandingInvoices = useMemo(() =>
    invoices.filter((i) => { const s = (i.status ?? '').toLowerCase(); return s !== 'paid' && s !== 'void' && s !== 'uncollectible' }),
    [invoices])
      ...
      <div className="max-w-2xl mx-auto px-6 pt-6 space-y-4">
        ...
        {activeProject ? (
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-5">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#7EC8A0' }}>Active Project</p>
                <h2 className="text-xl font-bold text-neutral-900 group-hover:text-black leading-snug">{activeProject.name}</h2>
                {activeProject.project_type && <p className="text-xs text-neutral-500 mt-0.5">{typeLabel(activeProject.project_type)}</p>}
              </div>
            ...
            {projectProgress.steps.length > 0 && (
              <div className="mb-5">
                    ...
                    return (
                      <div key={step.id} className="flex items-center" style={{ flex: isLast ? '0 0 auto' : 1 }}>
                        <div className="w-3 h-3 rounded-full shrink-0 transition-all"
                          style={{ background: allDone ? '#1A3428' : isActive ? '#F04D3D' : '#e5e5e5', outline: isActive ? '3px solid #F04D3D30' : 'none' }} />
                        {!isLast && <div className="h-0.5 flex-1" style={{ background: allDone ? '#1A3428' : '#e5e5e5' }} />}
                      </div>
                    )
                  })}
                </div>
                <div className="flex justify-between mt-2">
                  ...
                  <p className="text-xs font-semibold" style={{ color: '#F04D3D' }}>{projectProgress.percent}%</p>
                </div>
              </div>
            )}

            <div className="w-full bg-neutral-100 rounded-full h-1.5">
              <div className="h-1.5 rounded-full transition-all" style={{ width: `${projectProgress.percent}%`, background: '#F04D3D' }} />
            </div>
        ...
        {newItems.length > 0 && (
          <div className="bg-white border border-neutral-200 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-semibold text-neutral-900">New Since Your Last Login</h3>
                <p className="text-xs text-neutral-400 mt-0.5">Files and notes ready for you</p>
              </div>
              <span
            ...
            <div className="space-y-2">
              {newItems.map((item) => {
                ...
                const isLink = !isNote && !!item.embed_url
                const ext = isNote
                  ? 'MEETING NOTE'
                  : item.file_type?.split('/').pop()?.toUpperCase() ?? (isLink ? 'LINK' : 'FILE')

                const destination = isNote
                  ? `/dashboard/client/projects/${item.project_id}?tab=notes`
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{ background: isNote ? '#1A342815' : '#F04D3D10', color: isNote ? '#1A3428' : '#F04D3D' }}
                    >
                      {isNote ? '✎' : isLink ? '↗' : '◆'}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-neutral-400 mt-0.5">
                        <span className="font-semibold">{ext}</span> · {formatDate(item.created_at)}
                      </p>
                    </div>
        ...
        {projects.length > 1 && (
          <div className="bg-white border border-neutral-200 rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {projects.map((p) => (
                <button key={p.id} onClick={() => router.push(`/dashboard/client/projects/${p.id}`)}
